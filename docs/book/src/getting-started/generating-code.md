...
Field-only traits declared in the generated schema are emitted as structural
contracts in every target. Trait functions and impl-function bodies are accepted
by the language but deliberately rejected by code generation until their
cross-target projection is specified.
