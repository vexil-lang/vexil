...
The binary wire format hasn't changed since April 2026 and breaking it would require a major version bump, while the language specification remains a draft. The repository includes a 111-file conformance corpus (45 valid and 66 invalid) and cross-implementation byte vectors for Rust and TypeScript. BLAKE3 schema hashes help peers detect incompatible schemas before exchanging application data. Neither the wire format nor the corpus have been exercised by external implementations or independently audited yet.
...
Rust and TypeScript have full code generation with cross-language byte output covered by the compliance vectors. Go and Python targets are available, but verify generated byte output for your schema and target combination before shipping a cross-language protocol.
