...
fn golden_source_test(test_name: &str, source: &str) {
    ...
    let golden_path = golden_dir.join(format!("{test_name}.go"));
    ...
    let generated = vexil_codegen_go::generate(&compiled).expect("codegen failed");
...
fn typed_tombstone_emits_no_codec_read() {
    ...
    let generated = vexil_codegen_go::generate(&compiled).expect("Go codegen");
