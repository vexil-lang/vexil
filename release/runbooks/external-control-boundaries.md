...
**Mode:** read-only evidence collection and non-mutating workflow rehearsal for future automated paths. This runbook is not a Manifest, provider configuration, approval, or authorization to release.

## Scope and current status

Vexil has separately completed the manually protected canonical tag
`packages/runtime-go/v0.1.1`. Its retained [closeout](../closeouts/runtime-go-0-1-1-manual-protected-tag-2026-07-26.json)
and [public observation](../history/observations/observation-go-runtime-v0-1-1-publication-2026-07-26.json)
record that narrow outcome. They do not establish a GitHub Release, registry
upload, deployment, bot action, workflow Run, or general provider readiness.
...
- `docs.yml` builds documentation for pull requests with no Pages deployment permission, OIDC token, or protected environment. Its main-only `deploy` job has separate GitHub Pages authority through the `github-pages` environment; it has no release, tag, registry, or package authority.
...
No reusable workflow may run untrusted pull-request or fork code in a privileged `pull_request_target` context. Advisory and rehearsal jobs have no protected environment, production secret, registry identity, release token, tag, release, publication, or deployment authority.
...
Provider execution approval is distinct from, and cannot replace, detached Manifest approval. For a future target, incomplete target evidence remains its own named prerequisite; the sole-maintainer policy itself is not a release gate.
