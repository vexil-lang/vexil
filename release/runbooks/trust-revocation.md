...
| Identify the public provider-control category | `repository-administrator` | stop, revoke, contain | use a credential, change an environment, mutate a workflow | same public exercise record | Exact surface listed in `emergency-stop-runbook` | Epic 2 verified control evidence is missing | `repository-administrator` | yes |
...
All GitHub Actions, environment, registry, and administrator recovery controls are **unverified Epic 2 blockers**. This tabletop record must not be used to claim revocation was tested or compliant.
